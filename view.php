...
// if the system is linux-like, the let's show the convert command
if(PHP_SHLIB_SUFFIX == "so"){
    echo "<li><a ".($r_select=="convert" ? "id='current'" : "")." href='?select=convert&amp;hash=$r_hash'>Convert</a></li>\n";
}
if ($debugtab) {
...
echo "</ul>\n";
echo "</div>\n";
// convert to ipad format
if ($r_select == "convert"){
    require_once("convert.php");
    show_convert_list($thistorrent['directory']);

    /* convert_file($thistorrent['directory']); */
}
// Display file info...
if ($r_select=="files") {
